...
async function createPaypalOrder({ items, total, origin }) {
  ...
  const response = await fetch(`${paypalBaseUrl}/v2/checkout/orders`, {
    ...
    body: JSON.stringify({
      intent: 'CAPTURE',
      purchase_units: [{
        description: `Echo City Roleplay: ${items.map((item) => `${item.quantity}x ${item.name}`).join(', ')}`,
        amount: {
          currency_code: 'EUR',
...
async function sendDiscordOrder({ type, buyer, items, total, paymentStatus, paypalOrderId, paypalDetails, western, proofFile }) {
...
app.post('/api/paypal/create-order', async (req, res, next) => {
  try {
    ...
    const order = await createPaypalOrder({ items, total, origin: req.headers.origin || process.env.PUBLIC_SITE_URL || 'http://localhost:5173' })
    const approvalLink = order.links?.find((link) => link.rel === 'approve')?.href
    res.json({ orderId: order.id, approvalLink, total, items })
  } catch (error) {
    next(error)
  }
})
...
app.post('/api/paypal/capture-order', async (req, res, next) => {
  try {
    ...
    const discord = await sendDiscordOrder({ type: 'paypal', buyer, items, total, paymentStatus, paypalOrderId, paypalDetails })
    res.json({ ok: true, captureStatus: capture.status, discord })
  } catch (error) {
    next(error)
  }
})
